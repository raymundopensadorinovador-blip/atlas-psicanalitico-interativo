...
export default function LoginPage() {
  ...
  function destinoPorPerfil(role: UserRole) {
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(destinoPorPerfil(profile.role as UserRole));
  }
      ...
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-6xl items-center justify-center">
        <div className="w-full max-w-xl rounded-[2rem] border border-cyan-300/20 bg-white/[0.06] p-6 shadow-2xl shadow-cyan-950/30 backdrop-blur md:p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            Acesse sua área como administrador, professor ou aluno.
          </p>
          ...
          <p className="mt-6 text-center text-sm text-slate-300">
            Ainda não tem conta?{" "}
            <Link href="/cadastro" className="font-semibold text-cyan-200">
              Criar conta
            </Link>
          </p>
        </div>
